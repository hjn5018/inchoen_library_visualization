...
class IncheonLibraryAPIClient:
    ...
    API 인증키가 없거나 연결에 실패할 경우 로컬의 최신 백업 데이터(latest data) 파일로 대체하는 기능이 내장되어 있습니다.
    """
    ...
    def fetch_raw_data(self):
        ...
        if not self.has_valid_key():
            print("API 인증키가 설정되지 않았거나 유효하지 않습니다. 로컬 최신 데이터(latest data)로 대체합니다.")
            return None
    ...
    def get_library_dataframe(self):
        ...
        오픈 API 데이터를 가져오지 못하는 경우 real_api_response.json (latest data) 파일에서 데이터를 로드합니다.
        """
        raw_data = self.fetch_raw_data()
        is_latest_data = False
        
        if raw_data is None:
            print("로컬 최신 데이터(latest data) 파일 데이터를 대체하여 로드합니다.")
            # 실행 경로에 맞춰 백업 데이터 경로 탐색
            fallback_paths = ["real_api_response.json", "../real_api_response.json", "v3/real_api_response.json"]
                ...
                if os.path.exists(path):
                    try:
                        with open(path, encoding='utf-8') as f:
                            ...
                            is_latest_data = True
                            print(f"성공적으로 로컬 파일({path})에서 최신 백업 데이터를 로드했습니다.")
                            break
                    except Exception as e:
                        print(f"최신 데이터 로딩 오류 ({path}): {e}")
            if raw_data is None:
                print("경고: 로컬 최신 데이터(latest data) JSON 파일을 찾거나 로드할 수 없습니다. 빈 데이터를 사용합니다.")
                raw_data = []
                is_latest_data = True
            
        df = pd.DataFrame(raw_data)
        return self.clean_dataframe(df), is_latest_data
...
if __name__ == "__main__":
    ...
    df, is_latest = client.get_library_dataframe()
    print(f"최신 데이터(latest data) 모드 동작 여부: {is_latest}, 로드된 도서관 레코드 수: {len(df)}")
